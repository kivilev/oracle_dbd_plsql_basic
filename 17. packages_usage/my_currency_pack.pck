create or replace package my_currency_pack is
  ...
  function get_code_by_curr_id_rc(pi_curr_id my_currency.curr_id%type) return my_currency.iso_code%type result_cache;

  -- обычная реализация
  function get_code_by_curr_id_simple(pi_curr_id my_currency.curr_id%type) return my_currency.iso_code%type;
...
create or replace package body my_currency_pack is
  ...
  -- реализация с result_cache
  function get_code_by_curr_id_rc(pi_curr_id my_currency.curr_id%type)
  return my_currency.iso_code%type 
  result_cache 
  is
    v_out my_currency.iso_code%type := null;
  begin
  ...
  -- Обычная процедура
  function get_code_by_curr_id_simple(pi_curr_id my_currency.curr_id%type)
  return my_currency.iso_code%type
  is
    v_out my_currency.iso_code%type := null;
  begin
...
begin
  -- кэшируем справочник
  for i in (select * from my_currency) loop
